...
public class ChangePasswordController implements Initializable {
    ...
    private boolean isCurrentPassMatch() {
        ...
        if (user.getPassword().matches(pfCurrent.getText())) {
            ...
            alert.setTitle("Current password not matched");
            alert.setHeaderText("Current password not matched");
            alert.show();
        }
        return isValid;
    }
    ...
    private boolean isNotNull() {
        ...
        if (pfCurrent.getText().isEmpty() || pfNewPass.getText().isEmpty() || pfRePass.getText().isEmpty()) {
            ...
            alert.setContentText("All text fields are required. please fill all text fields");
            alert.show();
            isValid = false;
        } else {
            isValid = true;
    ...
    private boolean isPasswordsAreMatch() {
        ...
        if (pfNewPass.getText().matches(pfRePass.getText())) {
            ...
            Alert alert = new Alert(Alert.AlertType.ERROR);
            alert.setTitle("Given password are not matched");
            alert.setHeaderText("Your given password does not match");
            alert.show();
        }
        return isValid;
    }
